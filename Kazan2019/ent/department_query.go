...
func (dq *DepartmentQuery) QueryDepartmentLocation() *DepartmentLocationQuery {
	query := (&DepartmentLocationClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withDepartmentLocation: dq.withDepartmentLocation.Clone(),
...
func (dq *DepartmentQuery) WithDepartmentLocation(opts ...func(*DepartmentLocationQuery)) *DepartmentQuery {
	query := (&DepartmentLocationClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withDepartmentLocation = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withDepartmentLocation; query != nil {
		if err := dq.loadDepartmentLocation(ctx, query, nodes,
			func(n *Department) { n.Edges.DepartmentLocation = []*DepartmentLocation{} },
			func(n *Department, e *DepartmentLocation) {
				n.Edges.DepartmentLocation = append(n.Edges.DepartmentLocation, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (dq *DepartmentQuery) loadDepartmentLocation(ctx context.Context, query *DepartmentLocationQuery, nodes []*Department, init func(*Department), assign func(*Department, *DepartmentLocation)) error {
	...
	query.Where(predicate.DepartmentLocation(func(s *sql.Selector) {
		s.Where(sql.InValues(department.DepartmentLocationColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "DepartmentID" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil

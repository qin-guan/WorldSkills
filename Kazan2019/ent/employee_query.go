...
func (eq *EmployeeQuery) QueryAsset() *AssetQuery {
...
func (eq *EmployeeQuery) WithAsset(opts ...func(*AssetQuery)) *EmployeeQuery {
	query := (&AssetClient{config: eq.config}).Query()
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	...
	if query := eq.withAsset; query != nil {
		if err := eq.loadAsset(ctx, query, nodes,
			func(n *Employee) { n.Edges.Asset = []*Asset{} },
			func(n *Employee, e *Asset) { n.Edges.Asset = append(n.Edges.Asset, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EmployeeQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Asset)) error {
	...
	query.Where(predicate.Asset(func(s *sql.Selector) {
		s.Where(sql.InValues(employee.AssetColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "EmployeeID" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil

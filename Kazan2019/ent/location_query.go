...
func (lq *LocationQuery) QueryDepartmentLocation() *DepartmentLocationQuery {
	query := (&DepartmentLocationClient{config: lq.config}).Query()
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withDepartmentLocation: lq.withDepartmentLocation.Clone(),
...
func (lq *LocationQuery) WithDepartmentLocation(opts ...func(*DepartmentLocationQuery)) *LocationQuery {
	query := (&DepartmentLocationClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withDepartmentLocation = query
	return lq
}
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withDepartmentLocation; query != nil {
		if err := lq.loadDepartmentLocation(ctx, query, nodes,
			func(n *Location) { n.Edges.DepartmentLocation = []*DepartmentLocation{} },
			func(n *Location, e *DepartmentLocation) {
...
func (lq *LocationQuery) loadDepartmentLocation(ctx context.Context, query *DepartmentLocationQuery, nodes []*Location, init func(*Location), assign func(*Location, *DepartmentLocation)) error {
	...
	query.Where(predicate.DepartmentLocation(func(s *sql.Selector) {
		s.Where(sql.InValues(location.DepartmentLocationColumn, fks...))
	}))
